...
type mapStateEntry struct {
	...
	// passes metadata is stored via a pointer to limit the overhead on entries without any passes
	// metadata.
	passes *passMetas
...
func newAllowEntryWithLabels(lbls labels.LabelArray) mapStateEntry {
	return newMapStateEntry(0, types.MinPriority, types.MaxPriority, makeSingleRuleOrigin(lbls, ""), 0, 0, types.Allow, NoAuthRequirement)
}
...
func (ms *mapState) pruneCoveredNarrowerKey(k Key, v mapStateEntry, key Key, precedence types.Precedence, changes ChangeState) {
	...
	for pass := range v.Passes() {
		...
		if pass.precedence >= precedence || !sameTier {
	...
	} else if deletePassMeta || deleteEntry {
		// Have to keep the entry for either pass or allow/deny
		if deletePassMeta {
			v.passes = nil
		} else if deletePassEntry {
			*v.passes = slices.DeleteFunc(*v.passes, func(pass passMeta) bool {
				return pass.precedence < precedence
			})
		}
		if deleteEntry {
			v.Invalidate()
...
func (sp *keySlice) addKey(key Key) {
	*sp = append(*sp, key)
}

func (sp *keySlice) addNewKeys(l34Keys, doneKeys Keys) {
	for k := range l34Keys {
		if _, done := doneKeys[k]; !done {
			sp.addKey(k)
		}
	}
}
...
func (sp *keySlice) All() iter.Seq[Key] {
	return func(yield func(Key) bool) {
		for len(*sp) > 0 {
			...
			*sp = (*sp)[1:]
			// reset empty slice to reuse the data from the beginning, if new keys are pushed while yielding
			if len(*sp) == 0 {
				*sp = (*sp)[:0]
			}
			if !yield(key) {
...
// insertNonPass iterates over the LPM ancestors of the given keys for the given non-pass entry,
// and adds the possibly updated entry to the mapState, unless bailed.
//
// While iterating it makes note of any pass metadata:
//   - for covering entries elevates the precedence of the new entry to follow that of the pass
//     entry.
//   - for non-covering pass entries, where the new key has a wildcard identity and the pass key has
//     a specific identity, a new key is added with the ID from the pass key and
//     the more specific L4 key from the new key, with precedence elevated to follow the pass entry.
//
// Additional subset keys may need to be added due to higher tier pass entries:
//   - for key.Identity == 0, additional keys with specific IDs from pass entries must be added
//   - narrower L3/L4 pass entries must be added for all higher tier pass entries with narrower L4
//     match.
//
// In many cases the given key is actually not added (is "bailed out"), and only those additional
// L3/4 entries remain.
//
// The additional keys are added to 'keys' so that the main loop adds them in turn.
func (ms *mapState) insertNonPass(tierMaxPrecedence types.Precedence, key Key, entry mapStateEntry, changes ChangeState) {
	...
	for k, v := range ms.BroaderOrEqualKeys(key) {
		...
		for pass := range v.Passes() {
			...
			if pass.tierMinPrecedence > entry.Precedence {
				...
				if !isCoveringKey {
					...
					l34Keys[key.WithIdentity(k.Identity)] = struct{}{}
					break // skip remaining passes of higher tiers
				}
		...
		if v.IsValid() && (v.Precedence > entry.Precedence ||
			...
			if isCoveringKey {
				...
				doneKeys[key.WithIdentity(k.Identity)] = struct{}{}
			}
		}
	}
	...
	if len(passes) > 0 {
		...
		if precedence > bailPrecedence {
			...
			passEntry := entry
			passEntry.Precedence = precedence
			...
			ms.addKeyWithChanges(key, passEntry, changes)
			bail = true
		}
	}
	if !bail {
	...
	// don't even try adding them here, only adding if the applicable passes are not bailed.
	for key := range keys.All() {
		...
		for k, v := range ms.BroaderOrEqualKeys(key) {
			...
			// Bail if covered by an allow/deny key of higher precedence on a higher tier.
			if v.IsValid() && (v.Precedence > entry.Precedence ||
				// New deny entry is also bailed due to different covering deny key
				// of the same precedence, equal keys need to be merged
				entry.IsDeny() && v.Precedence == entry.Precedence && k != key) {
...
func (ms *mapState) insertWithPasses(tierMaxPrecedence types.Precedence, key Key, entry mapStateEntry, changes ChangeState) {
	...
	// Add a new allow or deny entry?
	if !entry.IsPassEntry() {
		ms.insertNonPass(tierMaxPrecedence, key, entry, changes)
		return
	}

	// else add a new pass entry
	...
	for _, v := range ms.CoveringBroaderOrEqualKeys(key) {
		...
		// bail if covered by a higher or equal precedence PASS entry on the same tier
		for oldPass := range v.Passes() {
			if oldPass.tierMinPrecedence == newPass.tierMinPrecedence &&
				oldPass.precedence >= newPass.precedence {
				return
			}
		}
	}
	// Delete covered entries of lower or same precedence levels at the same tier.
	// Note that this pass entry is always added before any possibly passed-to lower tier
	// entries, so we are not accidentallly deleting any already passed to entries here.
	for k, v := range ms.CoveredNarrowerOrEqualKeys(key) {
		// Are all the pass entries of a lower precedence on the same tier?
		deletePassMeta := true
		deletePassEntry := false
		for pass := range v.Passes() {
			if pass.precedence > newPass.precedence ||
				pass.precedence < newPass.tierMinPrecedence {
				// higher precedence or on a lower tier,
				// cannot delete
				deletePassMeta = false
			} else {
				// lower or equal precedence on the same tier, have to delete
				deletePassEntry = true
			}
		}

		// Should the allow/deny verdict be deleted?
		deleteEntry := !v.IsValid() || v.Precedence < newPass.precedence
		...
		if deletePassMeta && deleteEntry {
			...
			if deletePassMeta {
				...
				*v.passes = slices.DeleteFunc(*v.passes, func(pass passMeta) bool {
					return pass.precedence <= newPass.precedence &&
						pass.precedence >= newPass.tierMinPrecedence
				})
			}
			if deleteEntry {
...
func (ms *mapState) determineAllowLocalhostIngress() {
	if option.Config.AlwaysAllowLocalhost() {
		...
		ms.insertWithChanges(types.Priority(0).ToTierMaxPrecedence(), localHostKey, entry, allFeatures, ChangeState{})
	}
}
